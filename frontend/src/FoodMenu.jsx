...
const FoodMenu = ({isblock}) => {
    ...
    useEffect(() => {
        ...
        if (restaurantId) {
            fetchRestaurantMenuById(restaurantId)
        }
    }, [searchParams])
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            let currentCategory = menu[0]?.Cuisine
            let minDistance = Number.POSITIVE_INFINITY

            menu.forEach((category) => {
                const element = sectionRefs.current[category?.Cuisine]
                if (element) {
                    ...
                    if (distance < minDistance) {
                        ...
                        currentCategory = category?.Cuisine
                    }
                }
            })
    ...
    const scrollToCategory = (categoryId) => {
        ...
        if (element) {
            const headerHeight = 170 // Increased to account for both sticky elements
            const rect = element.getBoundingClientRect()
            ...
            <div className="px-4 lg:px-20 min-h-screen ">
                    ...
                    <div className="flex overflow-x-auto hide-scrollbar gap-2 pb-2">
                        {loading ? (
                            ...
                            menu.map((category) => (
                                <Button
                                    key={category?._id}
                                    onClick={() => scrollToCategory(category?.Cuisine)}
                                    variant={activeCategory === category?.Cuisine ? "secondary" : "ghost"}
                                    className={`flex-shrink-0 whitespace-nowrap ${activeCategory === category?.Cuisine
                ...
                <div className="hidden lg:flex flex-col lg:flex-row gap-6">
                        ...
                        <div className="sticky top-24 bg-white p-4 border-r">
                            ...
                            {menu.map((category) => (
                                <motion.div
                                    key={category?._id}
                                    onClick={() => scrollToCategory(category?.Cuisine)}
                                    className={`cursor-pointer py-2 px-3 rounded-md transition-colors duration-200 ${activeCategory === category?.Cuisine ? "bg-orange-100 text-orange-600" : "hover:bg-gray-50"
                                        }`}
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                >
                                    <span className="font-medium">{category?.Cuisine}</span>
                                </motion.div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="lg:w-1/2">
                        {menu.map((category) => (
                            <div key={category?._id} ref={(el) => (sectionRefs.current[category?.Cuisine] = el)} className="mb-8">
                                <h2 className="text-2xl font-bold mb-4">{category?.Cuisine}</h2>
                                <div className="grid gap-6">
                                    <AnimatePresence>
                                        {filteredItems
                                            .filter((item) => item?.Cuisine === category?.Cuisine)
                                            .map((item) => (
                                                <motion.div
                                                    key={item?._id}
                                                    ...
                                                    <Card className="overflow-hidden hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                                                        <CardContent className="p-4">
                                                            <div className="flex flex-col sm:flex-row gap-4 items-start">
                                                                <img
                                                                    src={item?.images?.[0] || "/no-image-food-placeholder.webp"}
                                                                    alt={item?.name}
                ...
                <div className="lg:hidden mt-5">
                    ...
                    <div className="grid gap-4 mb-20">
                        {menu.map((category) => (
                            <div key={category?._id} ref={(el) => (sectionRefs.current[category?.Cuisine] = el)} className="mb-8">
                                <h2 className="text-xl font-bold mb-4">{category?.Cuisine}</h2>
                                            ...
                                            <Card key={item?._id} className="overflow-hidden">
                                                <CardContent className="p-4">
                                                    <div className="flex flex-col gap-4">
                                                        <img
                                                            src={item?.images.length > 0 ? item?.images?.[0] : "/no-image-food-placeholder.webp"}
                                                            alt={item?.name}
